...
__global__ void invertImageKernel(uint8_t* image_array, uint8_t* new_image_array) {
...
int main (void) {
    ...
    cudaMemcpy(d_image_array, h_image_array, M*N*C*sizeof(uint8_t), cudaMemcpyHostToDevice);
    ...
    for (int i = 1; i < 200; i++) {
        ...
        // invert
        int blockSize = i;
        int numBlocks = (((M * N * C)+15)/16 + blockSize - 1) / blockSize;
        cudaEventRecord(start, 0);
        invertImageKernel<<<numBlocks, blockSize>>>(d_image_array, d_new_image_array);
        cudaEventRecord(stop, 0);
        cudaEventSynchronize(stop);
        float time;
        cudaEventElapsedTime(&time, start, stop);
        std::cout << "Threads/block = " << i << ", Time: " << time << " ms\n";
